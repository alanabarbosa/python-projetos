start = input('Quer começar? (S/N) ').upper()
...
def quiz():
    if start == 'S':
        ...
        questions = {
            ...
            "Questão 02: \nComo verificar se o que o usuário digitou no input é uma string em Python? \nPara verificar se o que o usuário digitou no input é uma string em Python, você pode usar a função instance() ou a função type(). A função instance() verifica se um objeto é uma instância de uma classe ou de uma subclasse. A função type() retorna o tipo do objeto. Para mais informações, consulte a minha resposta anterior." : False,
            "Questão 03: \nO que é PEP 8? \nPEP 8 é um guia de estilo para código Python. Ele define as diretrizes para escrever código Python legível e consistente. O guia abrange tópicos como convenções de nomenclatura, indentação, comentários ignorando os espaçamentos em branco.": False,
